...
export class TestEntryPage implements OnInit {
  ...
  get canComplete(): boolean {
    if (!this.entry) return false;
    return this.entry.results.every(r => {
      const val = this.scores[r.exerciseId];
      return val !== null && val !== undefined && !isNaN(val) && !this.errorFor(r.exerciseId);
    });
  }
  ...
  async handleRefresh(event: CustomEvent) {
    await new Promise<void>(resolve => {
      this.service.getEntry(this.sessionId, this.tsgId).subscribe({
        next: entry => { this.entry = entry; this.scores = this.buildScores(entry.results); resolve(); },
        error: () => { this.showToast('Failed to refresh', 'danger'); resolve(); },
      });
    });
...
async confirmComplete() {
  ...
  onBlur(exerciseId: string) {
    this.blurTimer = setTimeout(() => { this.focusedIndex = -1; }, 150);
    const val = this.scores[exerciseId];
    if (val === null || val === undefined) {
      this.service.deleteResult(this.sessionId, this.tsgId, exerciseId).subscribe();
    } else {
      this.autoSave();
    }
  }
  ...
  private autoSave() {
    if (!this.entry || this.saving) return;
    const results = this.entry.results
      .filter(r => this.scores[r.exerciseId] !== null && this.scores[r.exerciseId] !== undefined && !this.errorFor(r.exerciseId))
      .map(r => ({ exerciseId: r.exerciseId, rawValue: this.scores[r.exerciseId]! }));
